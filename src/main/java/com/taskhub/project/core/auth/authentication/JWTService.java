...
public class JWTService {
    ...
    public static class DecodedToken {
        ...
        private final RoleWithActions roleWithActions;
        ...
        private DecodedToken(Claims claims) throws MalformedJwtException {
            ...
            // logic for roles
            var rawRoles = (LinkedHashMap<String, Object>) claims.get("role");

            if (rawRoles != null) {
                var action = (List<String>) rawRoles.get("actions");

                roleWithActions = RoleWithActions.builder()
                        .workSpaceId(rawRoles.get("workSpaceId").toString())
                        .actions(action)
                        .build();
            } else {
                roleWithActions = null;
            }
        }
    }
    ...
    @NoArgsConstructor
    @Data
    public static class RoleWithActions {
        private String workSpaceId;
        private List<String> actions;

        public static RoleWithActions of(Role role, String workSpaceId) {
            RoleWithActions roleWithActions = new RoleWithActions();
            roleWithActions.setActions(List.of(role.getActionCode().split(",")));
            roleWithActions.setWorkSpaceId(workSpaceId);
            return roleWithActions;
        }
    }
